...
namespace o2
{
	...
	// -----------------------------------------------
	template<typename T>
	class TAssetMeta : public AssetMeta
	{
	public:
		// Returns asset type id
		const Type* GetAssetType() const override { return &TypeOf(T); }

		SERIALIZABLE(TAssetMeta<T>);
	};
}
...
CLASS_BASES_META(o2::TAssetMeta<T>)
{
	BASE_CLASS(o2::AssetMeta);
}
END_META;
META_TEMPLATES(typename T)
CLASS_FIELDS_META(o2::TAssetMeta<T>)
{
}
END_META;
META_TEMPLATES(typename T)
CLASS_METHODS_META(o2::TAssetMeta<T>)
{
