...
namespace o2
{
	...
	{
		mInfo.tree = !isEditor ?
			Ref(&o2Assets.GetAssetsTree()) :
			o2Assets.GetAssetsTrees().FindOrDefault([](const Ref<AssetsTree>& x) { return x != Ref(&o2Assets.GetAssetsTree()); });
	}

	bool Asset::IsEditorAsset() const
	{
		return mInfo.tree != Ref(&o2Assets.GetAssetsTree());
	}
#endif
