...
namespace o2
{
	...
	void VectorFontAsset::UpdateFontEffects()
	{
		...
		{
			if (eff)
				clonedEffects.Add(Ref(eff->CloneAs<VectorFont::Effect>()));
		}

		StaticCast<VectorFont>(mFont)->SetEffects(clonedEffects);
	}
}
