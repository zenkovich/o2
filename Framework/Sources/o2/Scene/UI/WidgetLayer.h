...
CLASS_FIELDS_META(o2::WidgetLayer)
{
	...
	PROTECTED_FIELD(mDepth).DEFAULT_VALUE(0.0f).SERIALIZABLE_ATTRIBUTE();
	PROTECTED_FIELD(mAbsolutePosition);
	PROTECTED_FIELD(mInteractableArea);
	PROTECTED_FIELD(mOwnerWidget).DEFAULT_VALUE(nullptr);
	PROTECTED_FIELD(mParent).DEFAULT_VALUE(nullptr);
	PROTECTED_FIELD(mChildren).SERIALIZABLE_ATTRIBUTE();
	PROTECTED_FIELD(mUpdatingLayout).DEFAULT_VALUE(false);
	PROTECTED_FIELD(mIsLocked).DEFAULT_VALUE(false);
	PROTECTED_FIELD(mUID).DEFAULT_VALUE(Math::Random()).SERIALIZABLE_ATTRIBUTE();
}
END_META;
