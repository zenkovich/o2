...
namespace o2
{
	...
	ContextMenu* ContextMenu::CreateItemsByPath(WString& path)
	{
		...
		while (true)
		{
			...
			Widget* subChild = resultContext->mItemsLayout->mChildWidgets.FindMatchOrDefault([&](auto x) {
...
// 		}
	...
	void ContextMenu::RemoveItem(const WString& path)
	{
		...
		while (true)
		{
			...
			Widget* subChild = targetContext->mItemsLayout->mChildWidgets.FindMatchOrDefault([&](auto x) {
				if (auto text = x->template GetLayerDrawable<Text>("basic/caption"))
					return text->text == subMenu;
		...
		Widget* removingItem = targetContext->mItemsLayout->mChildWidgets.FindMatchOrDefault([&](auto x) {
			if (auto text = x->template GetLayerDrawable<Text>("basic/caption"))
