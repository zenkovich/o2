...
CLASS_FIELDS_META(o2::GridLayout)
{
	...
	PROTECTED_FIELD(mBaseCorner).DEFAULT_VALUE(BaseCorner::Left).SERIALIZABLE_ATTRIBUTE();
	PROTECTED_FIELD(mSpacing).DEFAULT_VALUE(0.0f).SERIALIZABLE_ATTRIBUTE();
	PROTECTED_FIELD(mCellSize).DEFAULT_VALUE(Vec2F(50, 50)).SERIALIZABLE_ATTRIBUTE();
	PROTECTED_FIELD(mArrangeAxis).DEFAULT_VALUE(TwoDirection::Horizontal).SERIALIZABLE_ATTRIBUTE();
	PROTECTED_FIELD(mArrangeAxisMaxCells).DEFAULT_VALUE(5).SERIALIZABLE_ATTRIBUTE();
	PROTECTED_FIELD(mBorder).SERIALIZABLE_ATTRIBUTE();
	PROTECTED_FIELD(mFitByChildren).DEFAULT_VALUE(false).SERIALIZABLE_ATTRIBUTE();
}
END_META;
