...
CLASS_FIELDS_META(o2::CustomDropDown)
{
	...
	FIELD().NAME(onBeforeExpand).PUBLIC();
	FIELD().NAME(onSelectedPos).PUBLIC();
	FIELD().NAME(onSelectedItem).PUBLIC();
	FIELD().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(nullptr).NAME(mItemsList).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(Layout::BothStretch()).NAME(mClipLayout).PROTECTED();
	FIELD().NAME(mAbsoluteClip).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(10).NAME(mMaxListItems).PROTECTED();
}
END_META;
