...
CLASS_FIELDS_META(o2::Toggle)
{
	...
	PROTECTED_FIELD(mValue).DEFAULT_VALUE(false).SERIALIZABLE_ATTRIBUTE();
	PROTECTED_FIELD(mValueUnknown).DEFAULT_VALUE(false).SERIALIZABLE_ATTRIBUTE();
	PROTECTED_FIELD(mCaptionText).DEFAULT_VALUE(nullptr);
	PROTECTED_FIELD(mBackLayer).DEFAULT_VALUE(nullptr);
	PROTECTED_FIELD(mToggleGroup).DEFAULT_VALUE(nullptr);
}
END_META;
