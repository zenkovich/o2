...
namespace o2
{
    ...
    public:
        ...
        tt(RefCounter* refCounter) : RefCounterable(refCounter), ScrollArea(refCounter) {}
        ...
        Vector<int> GetSelectedItems() const { return {}; }

        // Returns selected item widget
        Ref<Widget> GetSelectedItem() const { return nullptr; }

        // Selects item
        void SelectItem(const Ref<Widget>& item) {}
        // Returns items count
        int GetItemsCount() const { return 0;  }

        int GetSelectedItemPos() const { return 0; }
        ...
        struct Selection
        {
            ...
            bool operator==(const Selection& other) const { return false; }
        };
    ...
    // List view widget with selection
    // -------------------------------
    class CustomList2 : public ScrollArea
    {
    public:
        PROPERTIES(CustomList2);
        PROPERTY(Vector<int>, selectedItems, SetSelectedItems, GetSelectedItems); // Selected item widget property
        PROPERTY(Ref<Widget>, selectedItem, SelectItem, GetSelectedItem);         // Selected item widget
        PROPERTY(int, selectedItemPos, SelectItemAt, GetSelectedItemPos);         // Selected item position property
        GETTER(int, itemsCount, GetItemsCount);                                   // All items count getter
    ...
    public:
        // Default constructor
        explicit CustomList2(RefCounter* refCounter);

        // Copy-constructor
        CustomList2(RefCounter* refCounter, const CustomList2& other);

        // Destructor
        ~CustomList2();

        // Copy-operator
        CustomList2& operator=(const CustomList2& other);
        ...
        SERIALIZABLE(CustomList2);
        CLONEABLE_REF(CustomList2);
...
CLASS_BASES_META(o2::tt)
{
    BASE_CLASS(o2::ScrollArea);
}
END_META;
CLASS_FIELDS_META(o2::tt)
{
...
CLASS_METHODS_META(o2::tt)
{
    ...
    FUNCTION().PUBLIC().SIGNATURE(void, OnSelectionChanged);
}
END_META;
...
CLASS_BASES_META(o2::CustomList2)
{
    BASE_CLASS(o2::ScrollArea);
}
END_META;
CLASS_FIELDS_META(o2::CustomList2)
{
    FIELD().PUBLIC().NAME(selectedItems);
    FIELD().PUBLIC().NAME(selectedItem);
    FIELD().PUBLIC().NAME(selectedItemPos);
...
END_META;
CLASS_METHODS_META(o2::CustomList2)
{

    typedef const Function<bool(const Ref<Widget>&, const Ref<Widget>&)>& _tmp1;

    FUNCTION().PUBLIC().CONSTRUCTOR(RefCounter*);
    FUNCTION().PUBLIC().CONSTRUCTOR(RefCounter*, const CustomList2&);
    FUNCTION().PUBLIC().SIGNATURE(void, Update, float);
    FUNCTION().PUBLIC().SIGNATURE(void, Draw);
    FUNCTION().PUBLIC().SIGNATURE(void, SetItemSample, const Ref<Widget>&);
    FUNCTION().PUBLIC().SIGNATURE(const Ref<Widget>&, GetItemSample);
