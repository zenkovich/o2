...
CLASS_FIELDS_META(o2::Widget)
{
	...
	FIELD().DEFAULT_TYPE_ATTRIBUTE(o2::WidgetState).DONT_DELETE_ATTRIBUTE().DEFAULT_VALUE(nullptr).NAME(mFocusedState).PROTECTED();
	...
	FIELD().DEFAULT_TYPE_ATTRIBUTE(o2::WidgetState).DONT_DELETE_ATTRIBUTE().DEFAULT_VALUE(nullptr).NAME(mVisibleState).PROTECTED();
	FIELD().DEFAULT_VALUE(false).NAME(mIsClipped).PROTECTED();
	FIELD().NAME(mBounds).PROTECTED();
	FIELD().NAME(mBoundsWithChilds).PROTECTED();
	FIELD().NAME(layersEditable).PUBLIC();
	FIELD().NAME(internalChildrenEditable).PUBLIC();
}
END_META;
