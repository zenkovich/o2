...
{
	FIELD().NAME(bodyType).PUBLIC();
	FIELD().NAME(mass).PUBLIC();
	FIELD().NAME(inertia).PUBLIC();
	FIELD().NAME(linearVelocity).PUBLIC();
	FIELD().NAME(angularVelocity).PUBLIC();
	FIELD().NAME(linearDamping).PUBLIC();
	FIELD().NAME(angularDamping).PUBLIC();
	FIELD().NAME(gravityScale).PUBLIC();
	FIELD().NAME(isBullet).PUBLIC();
	FIELD().NAME(isSleeping).PUBLIC();
	FIELD().NAME(isFixedRotation).PUBLIC();
	FIELD().DEFAULT_VALUE(nullptr).NAME(mBody).PROTECTED();
	FIELD().NAME(mMassData).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(Type::Dynamic).NAME(mBodyType).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(1.0f).NAME(mMass).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(1.0f).NAME(mInertia).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(0.0f).NAME(mLinearDamping).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(0.0f).NAME(mAngularDamping).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(1.0f).NAME(mGravityScale).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(false).NAME(mIsBullet).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().DEFAULT_VALUE(false).NAME(mIsFixedRotation).PROTECTED();
	FIELD().NAME(mColliders).PROTECTED();
}
END_META;
