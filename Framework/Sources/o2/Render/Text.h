...
CLASS_FIELDS_META(o2::Text)
{
	...
	FIELD().NAME(wordWrap).PUBLIC();
	FIELD().NAME(dotsEngings).PUBLIC();
	FIELD().NAME(symbolsDistanceCoef).PUBLIC();
	FIELD().NAME(linesDistanceCoef).PUBLIC();
	FIELD().DEFAULT_VALUE(4096).NAME(mMeshMaxPolyCount).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().NAME(mText).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().NAME(mFontAssetId).PROTECTED();
	FIELD().NAME(mFont).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().NAME(mHeight).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().NAME(mSymbolsDistCoef).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().NAME(mLinesDistanceCoef).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().NAME(mVerAlign).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().NAME(mHorAlign).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().NAME(mWordWrap).PROTECTED();
	FIELD().SERIALIZABLE_ATTRIBUTE().NAME(mDotsEndings).PROTECTED();
	FIELD().NAME(mMeshes).PROTECTED();
	FIELD().NAME(mLastTransform).PROTECTED();
	FIELD().NAME(mSymbolsSet).PROTECTED();
	FIELD().NAME(mUpdatingMesh).PROTECTED();
}
END_META;
