...
namespace o2
{
	...
	{
		return mRects.Max<int>([&](Ptr<Rect> rt) { return rt->mPage; });
	}
	...
	bool RectsPacker::Pack()
	{
		...
		mRects.Sort([](auto a, auto b) { return a->mSize.x*a->mSize.y > b->mSize.x*b->mSize.y; });
	...
	bool RectsPacker::TryInsertRect(Rect& rt, QuadNode& node)
	{
			...
			node.mRect.Height() >= rt.mSize.y)
		{
			...
			node.mFree = false;
			node.AddChild(mnew QuadNode(node.mPage, RectF(leftDown.x, rightTop.y, center.x, center.y)));
			node.AddChild(mnew QuadNode(node.mPage, RectF(center.x, rightTop.y, rightTop.x, center.y)));
