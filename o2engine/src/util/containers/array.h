...
public:
	...
	public:
		ConstIterator(const Array* const arr = NULL, int index = 0);
...
public:
	...
	void Sort(bool(*compareFunc)(_type&, _type&) = &CompareLess);
...
_type& Array<_type>::Iterator::Value()
{
	...
	return mArray->mValues[mIndex];
}


//implementation array::citerator
template<typename _type>
Array<_type>::ConstIterator::ConstIterator(const Array<_type>* const arr /*= NULL*/, int index /*= 0*/):
mArray(arr), mIndex(index)
{
}
...
template<typename _type>
typename Array<_type>::ConstIterator& Array<_type>::ConstIterator::operator++() // ++A;
{
	mIndex++;
	return *this;
}
...
template<typename _type>
typename Array<_type>::ConstIterator Array<_type>::ConstIterator::operator++(int) // A++;
{
	Iterator temp = *this;
	mIndex++;
	return temp;
...
template<typename _type>
bool Array<_type>::ConstIterator::operator>(const ConstIterator& itr)
{
	return mIndex > itr->mIndex;
}
