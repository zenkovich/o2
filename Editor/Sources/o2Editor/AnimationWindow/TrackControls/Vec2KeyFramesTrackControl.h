...
{
    FIELD().PRIVATE().NAME(Ref<SplineTool>).NAME(mTool);
    FIELD().PRIVATE().NAME(Ref<IEditTool>).DEFAULT_VALUE(nullptr).NAME(mPrevSelectedTool);
    FIELD().PRIVATE().NAME(WeakRef<Actor>).DEFAULT_VALUE(nullptr).NAME(mTrackOwner);
}
END_META;
...
CLASS_METHODS_META(Editor::Vec2KeyFramesTrackControl)
{
    ...
    FUNCTION().PUBLIC().CONSTRUCTOR(const Ref<Vec2KeyFramesTrackControl>&);
