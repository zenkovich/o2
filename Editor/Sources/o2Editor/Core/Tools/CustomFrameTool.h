...
namespace Editor
{
    ...
    public:
        ...
        // Sets frame
        void SetFrame(const Ref<const Basis>& frame);

        // Returns current frame
        const Ref<const Basis>& GetFrame() const;
    ...
    private:
        Ref<const Basis> mBasis; // Current editing basis
    };
}
...
CLASS_METHODS_META(Editor::CustomFrameTool)
{
    ...
    FUNCTION().PUBLIC().SIGNATURE(void, SetFrame, const Ref<const Basis>&);
    FUNCTION().PUBLIC().SIGNATURE(const Ref<const Basis>&, GetFrame);
