...
namespace Editor
{
	...
	{
		for (const auto& instance : tool->mSkeletons)
	...
	SkeletonTool::BoneHandle::BoneHandle()
	{
		...
		auto drawHandleRegular = [=](const Basis& transform, const Color4& color) {
			...
			o2Render.DrawBuffer(PrimitiveType::Polygon, vertices.Data(), vertices.Count(), indexes, 4, Ref<Texture>::Null());
...
namespace Editor
{
	...
	public:
		...
		Ref<FunctionalRectDrawable> GetRegularDrawable();
		Ref<FunctionalRectDrawable> GetHoverDrawable();
		Ref<FunctionalRectDrawable> GetPressedDrawable();

		Vec2F GetDraggingBeginPosition();
		Vec2F GetDraggingOffset();

		Ref<FunctionalRectDrawable> drawHandleRegular;
		Ref<FunctionalRectDrawable> drawHandleHover;
		Ref<FunctionalRectDrawable> drawHandlePressed;

		Vec2F position;
		Mode mode;

		std::function<void()> onPressed;
		std::function<void(const Vec2F&)> onChangedPos;
	};

	class BoneHandle
	{
	public:
		BoneHandle(BoneComponent* boneComponent);
		~BoneHandle();

		void UpdatePosition();
		void OnHandleChangedPos(const Vec2F& pos);

		Ref<SceneDragHandle> handle;

	private:
		BoneComponent* boneComponent;
		Basis pressedTransform;
	};
	...
	void SkeletonTool::BoneHandle::OnHandleChangedPos(const Vec2F& pos)
	{
		...
		const auto& compTransform = boneComponent->GetOwnerActor()->transform;
		const auto worldPos = compTransform->GetWorldPosition();
		const auto transform = Basis::Translated(worldPos * -1.0f)
			* Basis::Rotated(angle)
	...
	{
		const auto compTransform = boneComponent->GetOwnerActor()->transform;
		const float angle = compTransform->GetWorldBasis().GetAngle();
	...
	BoneHandle::BoneHandle(BoneComponent* boneComponent)
		: boneComponent(boneComponent)
	{
	...
	{
		for (const auto& handle : boneHandles)
