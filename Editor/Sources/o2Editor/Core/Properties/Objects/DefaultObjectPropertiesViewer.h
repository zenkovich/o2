...
namespace Editor
{
    ...
    protected:
        ...
        bool CheckBuildProperties(const Vector<Pair<Ref<IObject>, Ref<IObject>>>& targetObjets) override;
...
{
    FIELD().PROTECTED().DEFAULT_VALUE(Ref<IObject>(&TypeOf(IObject))).NAME(mRealObjectType);
    FIELD().PROTECTED().DEFAULT_VALUE(Ref<IObject>(&TypeOf(IObject))).NAME(mBuiltObjectType);
...
CLASS_METHODS_META(Editor::DefaultObjectPropertiesViewer)
{
    ...
    FUNCTION().PROTECTED().SIGNATURE(bool, CheckBuildProperties, const Ref<Vector<Pair<Ref<IObject>, Ref<IObject>>>>& targetObjets);
    FUNCTION().PROTECTED().SIGNATURE(void, OnRefreshed, const Ref<Vector<Pair<Ref<IObject>, Ref<IObject>>>>& targetObjets);
}
END_META;