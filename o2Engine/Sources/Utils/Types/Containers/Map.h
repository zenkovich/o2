...
#include "Utils/Types/Containers/IDictionary.h"

namespace o2
{
	// - Отнаследовать Map от std::map
	// - Изменить все typedef на using = 
	// - Изменить все XxxDict на XxxMap
	...
	{
	public:
		using KeyValuePair = std::pair<key_type, _value_type>;
	...
	template<typename _key_type, typename _value_type>
	Map<_key_type, _value_type>::Map(std::initializer_list<value_type> init):
