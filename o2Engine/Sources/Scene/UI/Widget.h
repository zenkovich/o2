...
namespace o2
{
	...
	public:
		...
		PROPERTY(bool, enabledForcibly, SetEnableForcible, IsEnabled); // Enable propertu, works forcibly @EDITOR_IGNORE

		PROPERTY(float, transparency, SetTransparency, GetTransparency); // Transparency property
		GETTER(float, resTransparency, GetResTransparency);              // Result transparency getter, depends on parent transparency @EDITOR_IGNORE
...
{
	PUBLIC_FIELD(enabledForcibly).EDITOR_IGNORE_ATTRIBUTE();
	PUBLIC_FIELD(transparency);
	PUBLIC_FIELD(resTransparency).EDITOR_IGNORE_ATTRIBUTE();
